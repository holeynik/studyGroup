...
Multiple_Plots + theme(strip.text.x = element_text(size=8, angle=0,face="bold"),
...
ggplot() +
  ...
  ) + facet_wrap(~scientific_name + reporting_status,
                 ncol = 2, # number of cols, alternatively `nrow=`
                 scales = "free_y", # standarize scales
                 strip.position = "top")
...
ggplot() +
  ...
  ) + facet_grid(scientific_name ~ reporting_status,
                 scales = "free") +
...
`ggplot2` combines with `gganimate` to create dynamic plots, perfect for presentations! *Note that this chunk needs extra packages that you will have to install and load before runing it*
