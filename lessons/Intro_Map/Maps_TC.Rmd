...
# usage
packages <- c("tidyverse", "sf", "sp", "rnaturalearth", "aquamapsdata")
ipak(packages)
...
ggplot(country110_sf) + geom_sf(aes(fill=as.factor(mapcolor7))) + theme(legend.position="none")
...
inset <- bc_country %>% 
  ggplot() + 
  geom_sf() + 
  annotate(geom="rect", ymax=49.4, ymin=49.7, xmin=-125, xmax=-124.65, color="red", alpha=0.2, size=1.2)
...
vp_inset<-viewport(width=0.3, height=0.3, x=0.2, y=0.2)
print(map, vp=vp_map)
...
bc_map <- map + 
  geom_point(data=df, aes(x=lon, y=lat, color=value)) 
...
bc_map +
  ...
  annotate("text", x=-124.9, y=49.45, label="Vancouver Isl.") + 
  annotate("text", x=-124.8, y=49.55, label="Other Isl.") 
